...
-- This is a vendored, modified version of a compatible commit from
-- the servant-client-ghcjs code at:
-- https://github.com/haskell-servant/servant/tree/master/servant-client-ghcjs
-- It supports specifying additional headers in the client context.

module App.XhrClient
  ( client,
    runClientM,
    ClientEnv (..),
    runClientMOrigin,
...
runClientM :: Seq.Seq Header -> ClientM a -> IO (Either ServantError a)
#ifdef __GHCJS__
runClientM extraH m = do
    ...
    runClientMOrigin m (ClientEnv (BaseUrl protocol hostname port "") extraH)
#else
...
performRequest req = do
  xhr <- liftIO initXhr
  burl <- asks baseUrl
  extraHeaders <- asks headers
  liftIO $ performXhr xhr burl (req{
    requestHeaders = (requestHeaders req) <> extraHeaders
  })
  resp <- toResponse xhr
